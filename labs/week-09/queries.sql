...
SELECT r.id, u.name AS requester_name, r.location, r.details, r.status
FROM requests r
JOIN users u ON r.requester_id = u.id
ORDER BY r.id;

-- ⑥ คำร้องเฉพาะของภาควิชาหนึ่ง  (JOIN + WHERE)
SELECT r.id, u.name AS requester_name, u.department, r.location, r.details, r.status
FROM requests r
JOIN users u ON r.requester_id = u.id
...
SELECT id, location, details, created_at
FROM requests
...
SELECT u.id, u.name, u.department, COUNT(r.id) AS request_count
FROM users u
